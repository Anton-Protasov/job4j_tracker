 ...
 * 1 - определим, что треугольник существует (метод exist): сумма его двух сторон должна быть больше третьей стороны
 * 2 - определим полупериметр треугольника (метод period) = (сумма сторон) /2
 * 3 - опрелеляем площадь треугольника (метод area), если, конечно, треугольник существует. Иначе rsl = -1;
