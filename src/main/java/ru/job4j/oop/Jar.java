...
3 - добавим в него метод public void pour(Jar another), который будет принимать объект типа Jar
4 - first.pour(second):
У объекта first вызываем метод pour. В нем есть поле this.value
- this - указывает на то что нужно взять значение этой переменной у объекта для которого мы вызываем метод.
