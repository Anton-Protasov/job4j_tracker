 ...
 * Метод public Item add(Item item) добавляет заявку, переданную в аргументах в массив
 * заявок items Метод public Item[] findAll() возвращает копию массива items без null
 * элементов (без пустых ячеек)
 * Метод public Item[] findByName(String key) проверяет в цикле все элементы массива items,
 * сравнивая name (используя метод getName класса Item) с аргументом метода String key
 * Элементы, у которых совпадает name, копирует в результирующий массив и возвращает его.
 * Метод public Item findById(int id) проверяет в цикле все элементы массива items, сравнивая
 * id с аргументом int id и возвращает найденный Item. Если Item не найден - возвращает null.
 *
 ...
 * По факту, мы не можем изменять размер массива, поэтому под удалением заявки здесь
 * подразумевается запись в ячейку нулевой ссылки - null. Теперь необходимо сжать массив -
 * избавиться от нулевых элементов.
 * В Java есть метод, который позволяет скопировать блоки массива целиком.
 * Перемещение в этом случае идет через
 * нативные методы операционной системы. Это значительно быстрее нежели
 * копировать отдельные ячейки.
 * System.arraycopy(source, startPos, dist, distPos, length);
 * source - массив откуда нужно скопировать элементы начиная с позиции startPos и до позиции
 * startPos + length.
...
//        items[size++] = item;
    ...
    distPos - индекс элемента в Arrays2 начиная с которого вставляем length
    элементов из массива Arrays1
    index - индекс обнуленного элемента, копируем все элементы, начиная со следующегго за ним.
    В ставляем их начиная с index
    Так же в конце нам нужно обнулить последнюю ячейку, так как она будет
    заполнена последним элементов, а мы элементы сдвинули
